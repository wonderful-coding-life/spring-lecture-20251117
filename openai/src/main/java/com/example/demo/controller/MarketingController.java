...
public class MarketingController {
    ...
    public String postMarketing(ProductForm product, Model model) {
        var systemMessage = """
                너는 전문 마케팅 카피라이터야.
        ...
        var userMessage = MessageFormat.format("""
                ### 입력 정보
                - 제품명: {0}
                - 가격: {1}
                - 구매 링크: {2}
                - 제품 특징: {3}
                """, product.getName(), product.getPrice(), product.getLink(), product.getFeatures());

        var marketing = chatModel.call(new SystemMessage(systemMessage), new UserMessage(userMessage));
